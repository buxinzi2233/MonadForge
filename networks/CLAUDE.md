...
- **LoKR** (`lokr.py::LoKRModule`) — thin MonadForge lifecycle wrapper over `lycoris.modules.lokr.LokrModule` (`lycoris-lora==3.4.0`). Factorization, initialization, bypass forward, dropout, and canonical `lokr_w*_a/b` state keys are official LyCORIS behavior. `lokr_full_factor=true` maps to LyCORIS `full_matrix=true`; full-full layouts force `alpha=lora_dim` and unit scale. The legacy `network_dim=114514` selector still replays with that same official unit-scale behavior, but the explicit flag is preferred.
